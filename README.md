...
go install github.com/kaka-milan-22/AnB/cmd/bob@v1.3.1
go install github.com/kaka-milan-22/AnB/cmd/alice@v1.3.1

# …or build from a local clone of the v1.3.1 tag
git clone --branch v1.3.1 https://github.com/kaka-milan-22/AnB.git && cd AnB
go build -o bin/bob   ./cmd/bob
go build -o bin/alice ./cmd/alice
```

Replace `v1.3.1` with `@latest` to track unreleased changes on `main`.
