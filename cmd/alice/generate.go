...
func cmdGen(args []string) error {
	fs := newFS("gen")
	style := fs.String("style", "apple", "apple | full | passphrase | pin | aes256")
	var length, count int
	fs.IntVar(&length, "l", 0, "size: apple=groups(1-8) full=chars(8-100) passphrase=words(3-12) pin=digits(4-32) aes256=bytes(32); 0=default")
